...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  ...
  for (let i = 0; i < arrOfArg.length; i++) {
    let arrSum = sum(arrOfArg[i], s);
    let arrMul = multiply(arrOfArg[i], m);
    s = arrSum[0];
    m = arrMul[0];
  }
...
function multiplyArray(multArr) { //eslint-disable-line
  ...
  let m = 1;
  for (let i = 0; i < 3; i++) {
    let arrMul = multiply(multArr[i], m);
    m = arrMul[0];
  }
...
function multiplyAnyArray(dynamicArray) { //eslint-disable-line
  ...
  let m = dynamicArray[0];
  let arg = dynamicArray[0];
  for (let i = 1; i < dynamicArray.length; i++) {
    let arrMul = multiply(dynamicArray[i], m);
    arg = arg + ',' + dynamicArray[i];
    m = arrMul[0];
  }
  return [m, `The numbers ${arg} have a product of ${m}.`];
